"""
Intensity control: preflight auto-exposure and split frames.
...
Both cameras that run the general cycle use this module: the ASI imager in its
``time`` and ``sun_cycle`` modes, and the Hamamatsu (``japan``) imager in
``sun_cycle`` only — its ``time`` mode is the plain programme it always was.
``cameras/asi/exposure.py`` is a re-export of this module, so the name the ASI
driver and its tests grew up with still resolves.

Nothing here is camera-specific. Full scale is the program's own 16-bit scale
(:data:`intensity.FULL_SCALE`), which both backends already deliver on: the
PICAM backend refuses a deeper camera and shifts a shallower one up
(``cameras/asi/camera.py``), and the Hamamatsu is read as MONO16 and put through
the same conversion (``cameras/japan/camera.py``).
...
# The scale every frame in this program arrives on: each backend refuses a
# camera deeper than 16 bits and shifts a shallower one up on the way out
# (``cameras/asi/camera.py``, ``cameras/japan/camera.py``), so full scale is a
# constant rather than something to sniff per frame.
