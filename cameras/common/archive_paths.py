...
Two cameras file frames this way: the ASI imager always, and the Hamamatsu
(``japan``) imager in its ``sun_cycle`` mode, where ``site_id`` is the name of
the observing site (``japan.location.name``) and ``device_id`` the instrument
name written into the ``NAME`` card (``japan.name``). ``cameras/asi/paths.py``
is a re-export of this module.
"""
