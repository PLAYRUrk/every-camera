...
* **Nothing waits without servicing the rest.** The gaps between exposures are
  ...
  published — see :meth:`AsiWorkerConsole._wait_until`. A focus frame is only
  taken when it comfortably fits before the next scheduled slot, so live view
  can never delay a measurement.
...
class AsiWorkerConsole(threading.Thread):
    ...
    def _hold_for_focus(self):
        ...
        while not self._stop_event.is_set() and self._service.focus_hold():
            if time.monotonic() - last_status >= 2.0:
                last_status = time.monotonic()
                cooling = self._refresh_camera_section()
                self._save_status(SETUP_STATUS, cooling=cooling)
            self._serve_params()
            # No slot to be late for, so no slack limit: the tool gets every
            # frame the camera can take.
    ...
    def _wait_for_start(self):
        ...
        while not self._started_event.is_set():
            cooling = self._refresh_camera_section()
            self._save_status("waiting", cooling=cooling)
            self._serve_params()
