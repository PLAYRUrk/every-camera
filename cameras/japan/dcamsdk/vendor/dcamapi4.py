...
# --- every-camera: the only change from the vendor drop ---------------------
# The vendor hard-codes one absolute path, which LD_LIBRARY_PATH cannot override
# and which gives no clue what went wrong when the SDK sits somewhere else. This
# tries DCAM_LIB first, then the vendor's own path, then the bare soname so a
# library already on the loader path is found, and on failure raises one error
# naming everything it tried — the same shape as cameras/asi/picam.py's _load().
#
# Every name added here keeps the __ prefix on purpose: dcam.py does
# "from dcamapi4 import *", and the leading double underscore is what keeps these
# out of it. At module level __x is not name-mangled, so __dll stays __dll and the
# prototype block further down needs no change.
import os

__platform_system = platform.system()
if __platform_system == 'Windows':
    __loader, __candidates = windll, ['dcamapi.dll']
else:  # Linux
    __loader = cdll
    __candidates = ['/usr/local/lib/libdcamapi.so', 'libdcamapi.so']

__override = os.environ.get('DCAM_LIB')
if __override:
    __candidates.insert(0, __override)

__dll = None
__errors = []
for __name in __candidates:
    try:
        __dll = __loader.LoadLibrary(__name)
        break
    except OSError as __exc:
        __errors.append('  %s: %s' % (__name, __exc))

if __dll is None:
    raise OSError(
        'Cannot load the DCAM-API runtime. Install the Hamamatsu DCAM-API SDK, '
        'or point DCAM_LIB at the library.\nTried:\n' + '\n'.join(__errors)
    )
# --- end -------------------------------------------------------------------
